...
* 채팅 오류 개선<br>
- 구글 아이디랑 미팅 이름이 다르면 내 메세지로 인식 못하는 오류 수정<br>
- 이름 순서대로 정렬되는 오류 수정<br>

* 출석 페이지 -> 지각자 순위로 업데이트
- 날짜 지난 약속들만 점수 반영하도록 함

* 못 한거
- 채팅은 모임코드로 하는 걸로 바꾸기 이전거 어디서 찾는지 모르겠다..
(새로 방 만들어서 모임마다 다르게 나오는거 확인 // 이전거 사라진건가?)
- 약속 설정했을 때 알림에서 모임이름 -> 모임코드로 나와버림,,
=> 수정한 부분이 많아서 못 한거 남겨두고 일단 한 거 먼저 올림
