 ...
 * 프로필 사진 추가<br>
 - 회원 정보창에서 추가 가능<br>
 -> 회원 정보 변경하고 저장안하고 나가도 그 정보가 화면에 남아있길래<br>
 db에 저장한 후에 sharedpreferences에 저장하도록 변경<br>
 - 채팅, 게시판에 프로필 반영<br>
 - 게시판 글쓴이 추가함<br>
...
// 2020.11.25(은아) <br>
 * UI수정<br>
 - 글자색, 버튼 모양 안 바뀐 부분들 수정<br>
 - 경로검색 버튼 안보이게<br>
 - 회원탈퇴시 확인창 뜨도록 수정<br>
 - 장소추천화면에서 시크바랑 스피너 디자인 변경<br>
 ...
 * UI 변경 <br>
 - 배경화면 펭귄이던거 지움 <br>
 - 버튼 테두리 색상 colorPrimary로 변경하고 글씨는 colorAccent로 지정 <br>
 - 캘린더 표시색 변경 : 확정 날짜 -> 테두리 / 오늘 -> 굵은 글씨 <br>

<br>
// 2020.11.23(은아)<br>
*정산 버튼 위치 수정<br>
*모임삭제 -> 이름으로 찾던거 코드로 변경<br>
...
<br>
// 2020.11.23(수현)<br>
* 출석 페이지 -> 지각자 순위로 업데이트<br>
- 날짜 지난 약속들만 점수 반영하도록<br>
=> schedule의 timePoint가 meetings의 best로 합산 됨(출첵 안한 사람은 자동으로 -12점)<br>
- 화면 전환 오류 수정 => listview로 5위까지 보여줌<br>
* 채팅 오류 개선<br>
- 구글 아이디랑 미팅 이름이 달라도 내 메세지로 인식하도록 수정<br>
- 보낸 순서대로 정렬 유지 되도록 수정<br>

<br>
// 2020.11.23(은아)<br>
* 모임 코드로 구분 <br>
- 모임 생성시 이름+설명 필수 입력 => 두 개가 모두 중복시 생성 불가<br>
- 모임 구분 - 코드 기준 // 모임 코드는 모임 선택할 때 바로 찾아서 전달전달 <br>
- 일정, 게시글 등 생성시 meetingID- 모임 코드가 저장되도록 <br>
* 못 한거<br>
 -  채팅은 모임코드로 하는 걸로 바꾸기 이전거 어디서 찾는지 모르겠다..<br>
    (새로 방 만들어서 모임마다 다르게 나오는거 확인 // 이전거 사라진건가?)<br>
 - 약속 설정했을 때 알림에서 모임이름 -> 모임코드로 나와버림,,<br>
 => 수정한 부분이 많아서 못 한거 남겨두고 일단 한 거 먼저 올림<br>
